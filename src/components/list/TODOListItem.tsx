...
function TODOListItem({ item, todos, setTodos }: { item: any, todos: any, setTodos: any }) {
    ...
    const handleDelete = () => {
        setTodos((prevTodos: any) => prevTodos.filter((todo: any) => todo.id !== item.id));

        // Update localStorage after deleting todo
        const updatedTodos = JSON.stringify(
            todos.filter((todo: any) => todo.id !== item.id)
        );
        localStorage.setItem("todos", updatedTodos);
    };
